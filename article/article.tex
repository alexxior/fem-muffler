...
\draw [<->]    (50,53.31) -- (79,53.31) ;
%Straight Lines [id:da733666892111086] 
\draw    (79,53.31) -- (79,64.31) ;
...
\end{tikzpicture}}
    ...
    \caption{Schemat tłumika refleksyjnego dla grupy 3.}
    \label{rysunek}
\end{figure}
\vspace{-0.2cm}
\begin{table}[ht]
...
\par Schemat procedury obliczeniowej znajduje się na \hyperref[rysunek2]{rysunku 2}, a listingi plików pod nim. Jako poprawne wywołanie \texttt{\hyperref[list1]{main.sh}} z poziomu \texttt{Bash}'a podaje się po nazwie argumenty kroków dyskretyzacji: $\Delta x_1, \Delta x_2, \Delta f$, zgodnie z notacją: \texttt{./main.sh krok\_x1 krok\_x2 krok\_f}\\
...
% Text Node
\draw (169,16.16) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize] [align=left] {\texttt{\hyperref[list1]{main.sh}}};
% Text Node
...
\label{list1}\lstinputlisting[language=Bash,caption=Główny skrypt wykonujący (\texttt{main.sh})]{../main.sh}
...
\newpage
\label{list2}\lstinputlisting[caption=Szablon modelu siatki \texttt{ElmerGrid} dla tłumika (\texttt{tlumik.grd})]{../data/tlumik.txt}
\label{list3}\lstinputlisting[caption=Szablon modelu siatki \texttt{ElmerGrid} dla płaskiego falowodu (\texttt{plaski.grd})]{../data/plaski.txt}
\label{list4}\lstinputlisting[caption=Szablon pliku obliczeniowego \texttt{case.sif} dla solvera MES: \texttt{ElmerSolver}]{../data/case.txt}
