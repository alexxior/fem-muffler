#!/bin/bash
if [[ $1 =~ "-h" ]]; then
  echo "RUN-MUFFLER Syntax: ./`basename $0` [x1step] [x2step] [fstep] [CCI|SWEEP](2x bool)"
  exit 0
fi
...
fstr=$(IFS=$' '; echo "${freq[*]}")
mkdir -p output/tlumik output/plaski
cd output
rm -f tlumik/* plaski/*
...
if [ $flag == 10 ] || [ $flag == 11 ]; then
	...
	octave --silent --eval "cd ../scripts; cci($x1min,$x1max,$x2min,$x2max)"
	while read -r x1 x2; do
		# obliczenia dla tlumika:
		xline=$(octave --silent --eval "cd ../scripts; replace($x1,$x2)")
		echo "          " $x1 "|" $x2 "| " $xline
	...
	sed -e 's/\s\+/,/g' tlumik/output.dat > cci-tlumik.txt
	sed -e 's/\s\+/,/g' plaski/output.dat > cci-plaski.txt
...
if [ $flag == 01 ] || [ $flag == 11 ]; then
	...
	sed -e 's/\s\+/,/g' tlumik/output.dat > sweep-tlumik.txt
	sed -e 's/\s\+/,/g' plaski/output.dat > sweep-plaski.txt
fi # wyznacz IL, optymalizuj i zwróć wynik w konsoli
echo -e "FEM finished: `date +%H:%M`\nPress Enter to close figures and print results in ./output/Optimum.txt"
octave --silent --eval "cd ../scripts; optimize($x1min,$1,$x1max,$x2min,$2,$x2max,$fmin,$fmax)" > Optimum.txt
cat Optimum.txt
cd ..