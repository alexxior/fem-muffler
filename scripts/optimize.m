function optimize(x1min,x1step,x1max,x2min,x2step,x2max,fmin,fmax)
    ...
    cd ../scripts
    CCI_intp2bez = sortcalc('cci-plaski.txt',fmin,fmax);
    % posortuj wyniki CCI i wyznacz calk strate wtracenia tlumika
    CCI_intp2tlum = sortcalc('cci-tlumik.txt',fmin,fmax);
    % wyznaczenie calkowitej straty wtracenia
    CCI_ILtot = 10*log10(CCI_intp2bez./CCI_intp2tlum);
    cd ../output
    ...
    cd ../scripts
    [SW_intp2bez, k] = sortcalc('sweep-plaski.txt',fmin,fmax);
    % posortuj wyniki sweepa i wyznacz calk strate wtracenia tlumika
    SW_intp2tlum = sortcalc('sweep-tlumik.txt',fmin,fmax);
    SW_ILtot = zeros(1,k);
    for i = 1:k
    ...
    cd ../output
    [SW_x1,SW_x2] = textread('sweep-x1x2.txt','%f %f');
    ...
    disp(num2str(a'));
   input([line,"Optimum for CCI:\nx1 (m)\tx2 (m)\tILtot (dB)\n",x1opt,"\t",x2opt,"\t",num2str(CCI_ILtotmax,2),"\n"]);